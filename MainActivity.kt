...
//                    myMap.animateCamera(CameraUpdateFactory.newLatLngZoom(latLng, 20f))
                    ...
                    var markerOptionDennis = myMap.addMarker(
                        MarkerOptions().position(latLng).title("Dennis")
        ...
        val logListener = object : ValueEventListener {
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if (dataSnapshot.exists()) {
                    //continiously get femi location and pass to the map reader
                    val femLiocationLogging =
                        dataSnapshot.child("FemiLocation").getValue(FemiLocationLogging::class.java)
                    val femiLat = femLiocationLogging?.Latitude
                    val femiLong = femLiocationLogging?.Longitude
                    ...
                    if (femiLat != null && femiLong != null) {
                        ...
                        femiMap.clear()
                        femiMap.addMarker(
                            MarkerOptions().position(femiLoc).title("Femi")
    ...
    //get my current location updates using FusedLocationProviderClient
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == LOCATION_PERMISSION_REQUEST) {
            if (grantResults.contains(PackageManager.PERMISSION_GRANTED)) {
                ...
                Toast.makeText(
                    this,
                    "You have not granted location access permission",
                    Toast.LENGTH_LONG
                ).show()
                finish()
            }
    ...
    // function that convert images and vector to mipmap for map icon usage
